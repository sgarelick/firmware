...
struct drv_can_rx_buffer_element {
	...
	__IO uint8_t DB[8 + 4 * CAN0_RX_BUFFERS_DATA_SIZE];
};
...
struct drv_can_rx_fifo_0_element {
	...
	__IO uint8_t DB[8 + 4 * CAN0_RX_FIFO_0_DATA_SIZE];
};
...
struct drv_can_rx_fifo_1_element {
	...
	__IO uint8_t DB[8 + 4 * CAN0_RX_FIFO_1_DATA_SIZE];
};
...
struct drv_can_tx_buffer_element {
	...
	__IO uint8_t DB[8 + 4 * CAN0_TX_DATA_SIZE];
};
...
struct drv_can_config {
	const struct drv_can_standard_filter * standard_filters;
	const struct drv_can_extended_filter * extended_filters;
